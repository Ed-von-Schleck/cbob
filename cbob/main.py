...
def main():
    ...
    parsers["subprojects_add"].add_argument("projects", metavar="project", nargs="+", help="Project(s) to be used as a sub-project.")
    ...
    parsers["subprojects_remove"].add_argument("projects", metavar="project", nargs="+", help="Subproject(s) to be removed.")
    parsers["subprojects_remove"].set_defaults(func=commands.subprojects_remove)

    parsers["depend"] = subparsers.add_parser("depend", help="Make a target depend on other targets.")
    parsers["depend"].add_argument("-t", "--target", help="The target that requires the dependencies (omit to mean default target).")
    parsers["depend"].add_argument("dependencies", metavar="dependency", nargs="+", help="The target(s) that are depended on.")
    parsers["depend"].set_defaults(func=commands.depend)
